...
class OauthUser {
  ...
  function verifyUser($userInfo) {
    ...
    if(!empty($userData)){
        ...
        $prevQuery = "SELECT * FROM ".$this->table." WHERE oauth_provider = '".$userData['oauth_provider']."' AND oauth_id = '".$userData['oauth_id']."'";
        ...
        if($prevResult->num_rows > 0){
            ...
            $query = "UPDATE ".$this->table." SET first_name = '".$userData['first_name']."' WHERE oauth_provider = '".$userData['oauth_provider']."' AND oauth_id = '".$userData['oauth_id']."'";
            $update = $this->db->query($query);
        }else{
            //Insert user data
            $query = "INSERT INTO ".$this->table." SET oauth_provider = '".$userData['oauth_provider']."', oauth_id = '".$userData['oauth_id']."', first_name = '".$userData['first_name']."', `status`='not_verified' ";
            $this->db->query($query);
            $_SESSION['status'] = "Active";
            $_SESSION['user_id']      = $gpUserProfile['id'];
            $_SESSION['first_name'] = $gpUserProfile['first_name'];
            header("location:home.php");
            exit();
        }
